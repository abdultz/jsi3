 ...
 */
public class SimpleIndex implements SpatialIndex3 {
  TIntObjectHashMap<Rectangle3> m_map = new TIntObjectHashMap<Rectangle3>();
   ...
   * Nearest
   */
  private TIntArrayList nearest(Point3 p, float furthestDistance) {
    TIntArrayList ret = new TIntArrayList();
    float nearestDistance = furthestDistance;
    TIntObjectIterator<Rectangle3> i = m_map.iterator();
    while (i.hasNext()) {
      i.advance();
      int currentId = i.key();
      Rectangle3 currentRectangle = i.value(); 
   ...
   */
  public void nearest(Point3 p, final TIntProcedure v, float furthestDistance) {
  ...
  private TIntArrayList nearestN(Point3 p, int n, float furthestDistance) {
    TIntArrayList ids = new TIntArrayList();
    TFloatArrayList distances = new TFloatArrayList();
    
    TIntObjectIterator<Rectangle3> iter = m_map.iterator();
    while (iter.hasNext()) {
      iter.advance();
      int currentId = iter.key();
      Rectangle3 currentRectangle = iter.value(); 
      float distance = currentRectangle.distance(p);
   ...
   */
  public void nearestN(Point3 p, final TIntProcedure v, int n, float furthestDistance) {
   ...
   */
  public void nearestNUnsorted(Point3 p, final TIntProcedure v, int n, float furthestDistance) {
