 ...
 */
public class SILWrapper implements SpatialIndex3 {
  
  private static final Logger log =  LoggerFactory.getLogger(SILWrapper.class);
  
  private IStorageManager3 storageManager = null; 
  private ISpatialIndex3 tree = null;
  private int size = 0;
  
  class IntProcedureVisitor implements IVisitor3 {
    ...
    public void visitNode(final INode3 n) {
      return;
    }
    
    public void visitData(final IData3 d) {
  ...
  public void init(Properties props) {
    ...
    storageManager = new MemoryStorageManager3();
    PropertySet3 propertySet = new PropertySet3();
    ...
    propertySet.setProperty("Dimension", new Integer(3));
    
    String strTreeVariant = props.getProperty("TreeVariant");
    Integer intTreeVariant = null;
    if (strTreeVariant.equalsIgnoreCase("Linear")) {
      intTreeVariant = new Integer(sil.spatialindex.SpatialIndex3.RtreeVariantLinear);
    } else if (strTreeVariant.equalsIgnoreCase("Quadratic")) {
      intTreeVariant = new Integer(sil.spatialindex.SpatialIndex3.RtreeVariantQuadratic); 
    } else {
      // default
      ...
      intTreeVariant = new Integer(sil.spatialindex.SpatialIndex3.RtreeVariantRstar); 
    }
    propertySet.setProperty("TreeVariant", intTreeVariant);
    
    tree = new RTree3(propertySet, storageManager);
  }
  ...
  public void nearest(Point3 p, TIntProcedure v, float furthestDistance) {
    tree.nearestNeighborQuery(1, 
                              new sil.spatialindex.Point3(new double[] {p.x, p.y, p.z}),  
  ...
  public void intersects(Rectangle3 r, TIntProcedure v) {
    Region3 region = new Region3(new double[] {r.minX, r.minY, r.minZ}, new double[] {r.maxX, r.maxY, r.maxZ});  
