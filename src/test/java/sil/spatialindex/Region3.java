...
public class Region3 implements IShape
{
	...
	public Region3(final double[] pLow, final double[] pHigh)
	{
	...
	public Region3(final Point3 low, final Point3 high)
	{
	...
	public Region3(final Region3 r)
	{
	...
	public boolean equals(Object o)
	{
		if (o instanceof Region3)
		{
			...
			{
				if (m_pLow[cIndex] < r.m_pLow[cIndex] - SpatialIndex3.EPSILON || m_pLow[cIndex] > r.m_pLow[cIndex] + SpatialIndex3.EPSILON ||
						m_pHigh[cIndex] < r.m_pHigh[cIndex] - SpatialIndex3.EPSILON || m_pHigh[cIndex] > r.m_pHigh[cIndex] + SpatialIndex3.EPSILON)
						return false;
			}
			return true;
		}
	...
	public Object clone()
	{
		return new Region3(m_pLow, m_pHigh);
	}
	...
	{
		if (s instanceof Region3) return intersects((Region3) s);

		if (s instanceof Point3) return contains((Point3) s);
	...
	{
		if (s instanceof Region3) return contains((Region3) s);
	...
	{
		if (s instanceof Region3) return touches((Region3) s);

		if (s instanceof Point3) return touches((Point3) s);
	...
	{
		if (s instanceof Region3) return getMinimumDistance((Region3) s);

		if (s instanceof Point3) return getMinimumDistance((Point3) s);

		throw new IllegalStateException("getMinimumDistance: Not implemented yet!");
	}

	public boolean intersects(final Region3 r)
	{
	...
	public boolean contains(final Region3 r)
	{
	...
	public boolean touches(final Region3 r)
	{
	...
	public double getMinimumDistance(final Region3 r)
	{
	...
	public boolean contains(final Point3 p)
	{
	...
	public boolean touches(final Point3 p)
	{
		if (m_pLow.length != p.m_pCoords.length) throw new IllegalArgumentException("touches: Shape has the wrong number of dimensions.");

		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
		{
			if ((m_pLow[cIndex] > p.m_pCoords[cIndex] - SpatialIndex3.EPSILON && m_pLow[cIndex] < p.m_pCoords[cIndex] + SpatialIndex3.EPSILON) ||
					(m_pHigh[cIndex] > p.m_pCoords[cIndex] - SpatialIndex3.EPSILON && m_pHigh[cIndex] < p.m_pCoords[cIndex] + SpatialIndex3.EPSILON))
					return true;
		}
		return false;
	}

	public double getMinimumDistance(final Point3 p)
	{
	...
	public double getIntersectingArea(final Region3 r)
	{
	...
	public Region3 combinedRegion(final Region3 r)
	{
		...
		return new Region3(mn, mx);
	}

	public static Region3 combinedRegion(Region3[] pRegions)
	{
	...
	public static void combinedRegion(Region3 pToModify, final Region3 pConst)
	{
