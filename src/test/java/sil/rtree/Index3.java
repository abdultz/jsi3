...
public class Index3 extends Node3
{
	public Index3(RTree3 pTree, int id, int level)
	{
	...
	protected Node3 chooseSubtree(Region3 mbr, int level, Stack pathBuffer)
	{
		...
		Node3 n = m_pTree.readNode(m_pIdentifier[child]);
		Node3 ret = n.chooseSubtree(mbr, level, pathBuffer);
	...
	protected Leaf3 findLeaf(Region3 mbr, int id, Stack pathBuffer)
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			{
				Node3 n = m_pTree.readNode(m_pIdentifier[cChild]);
				Leaf3 l = n.findLeaf(mbr, id, pathBuffer);
				if (l != null) return l;
			}
		}
	...
	protected Node3[] split(byte[] pData, Region3 mbr, int id)
	{
		...
		Node3 left = new Index3(m_pTree, m_identifier, m_level);
		Node3 right = new Index3(m_pTree, -1, m_level);
	...
	protected int findLeastEnlargement(Region3 r)
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			Region3 t = m_pMBR[cChild].combinedRegion(r);
	...
	protected int findLeastOverlap(Region3 r)
	{
		...
		if (me < SpatialIndex3.EPSILON || me > SpatialIndex3.EPSILON)
		{
			int cIterations;
	...
	protected void adjustTree(Node3 n, Stack pathBuffer)
	{
		...
		m_pMBR[child] = (Region3) n.m_nodeMBR.clone();
	...
	protected void adjustTree(Node3 n1, Node3 n2, Stack pathBuffer, boolean[] overflowTable)
	{
		m_pTree.m_stats.m_adjustments++;
		...
		m_pMBR[child] = (Region3) n1.m_nodeMBR.clone();
		...
		boolean adjusted = insertData(null, (Region3) n2.m_nodeMBR.clone(), n2.m_identifier, pathBuffer, overflowTable);
