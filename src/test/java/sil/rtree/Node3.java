...
abstract class Node3 implements INode3
{
	protected RTree3 m_pTree = null;
		// Parent of all nodes.
	...
	protected abstract Node3 chooseSubtree(Region3 mbr, int level, Stack pathBuffer);
	protected abstract Leaf3 findLeaf(Region3 mbr, int id, Stack pathBuffer);
	protected abstract Node3[] split(byte[] pData, Region3 mbr, int id);
	...
	public IShape getChildShape(int index) throws IndexOutOfBoundsException
	{
		...
		return new Region3(m_pMBR[index]);
	}
	...
	protected Node3(RTree3 pTree, int id, int level, int capacity)
	{
		m_pTree = pTree;
		m_level = level;
		m_identifier = id;
		m_capacity = capacity;
		m_nodeMBR = (Region3) pTree.m_infiniteRegion.clone();

		m_pDataLength = new int[m_capacity + 1];
		m_pData = new byte[m_capacity + 1][];
		m_pMBR = new Region3[m_capacity + 1];
		m_pIdentifier = new int[m_capacity + 1];
	}

	protected void insertEntry(byte[] pData, Region3 mbr, int id) throws IllegalStateException
	{
		...
		Region3.combinedRegion(m_nodeMBR, mbr);
	}
	...
	protected void deleteEntry(int index) throws IndexOutOfBoundsException
	{
		...
		if (m_children == 0)
		{
			m_nodeMBR = (Region3) m_pTree.m_infiniteRegion.clone();
		}
		else if (touches)
		{
	...
	protected boolean insertData(byte[] pData, Region3 mbr, int id, Stack pathBuffer, boolean[] overflowTable)
	{
		if (m_children < m_capacity)
		{
			...
			if (! b && ! pathBuffer.empty())
			{
				...
				Index3 p = (Index3) m_pTree.readNode(cParent);
		...
		else if (m_pTree.m_treeVariant == SpatialIndex3.RtreeVariantRstar && ! pathBuffer.empty() && overflowTable[m_level] == false)
		{
		...
		{
			Node3[] nodes = split(pData, mbr, id);
			Node3 n = nodes[0];
			Node3 nn = nodes[1];
			...
			if (pathBuffer.empty())
			{
				...
				Index3 r = new Index3(m_pTree, m_pTree.m_rootID, m_level + 1);

				r.insertEntry(null, (Region3) n.m_nodeMBR.clone(), n.m_identifier);
				r.insertEntry(null, (Region3) nn.m_nodeMBR.clone(), nn.m_identifier);
	...
	protected void reinsertData(byte[] pData, Region3 mbr, int id, ArrayList reinsert, ArrayList keep)
	{
	...
	protected void rtreeSplit(byte[] pData, Region3 mbr, int id, ArrayList group1, ArrayList group2)
	{
		int cChild;
		...
		Region3 mbr1 = (Region3) m_pMBR[seeds[0]].clone();
		Region3 mbr2 = (Region3) m_pMBR[seeds[1]].clone();
		...
		while (cRemaining > 0)
		{
			...
			else
			{
				...
				for (cChild = 0; cChild < m_capacity + 1; cChild++)
				{
					if (mask[cChild] == false)
					{
						Region3 a = mbr1.combinedRegion(m_pMBR[cChild]);
						d1 = a.getArea() - a1;
						Region3 b = mbr2.combinedRegion(m_pMBR[cChild]);
						d2 = b.getArea() - a2;
						d = Math.abs(d1 - d2);
						...
						if (d > m)
						{
							...
							md1 = d1; md2 = d2;
							sel = cChild;
							if (m_pTree.m_treeVariant== SpatialIndex3.RtreeVariantLinear || m_pTree.m_treeVariant == SpatialIndex3.RtreeVariantRstar) break;
						}
					}
				}
				...
				if (group == 1)
				{
					Region3.combinedRegion(mbr1, m_pMBR[sel]);
				}
				else
				{
					Region3.combinedRegion(mbr2, m_pMBR[sel]);
				}
			}
		}
	}

	protected void rstarSplit(byte[] pData, Region3 mbr, int id, ArrayList group1, ArrayList group2)
	{
		...
		for (cDim = 0; cDim < m_pTree.m_dimension; cDim++)
		{
			...
			for (cChild = 1; cChild <= splitDistribution; cChild++)
			{
				...
				Region3[] tl1 = new Region3[l];
				Region3[] th1 = new Region3[l];
				...
				Region3 bbl1 = Region3.combinedRegion(tl1);
				Region3 bbh1 = Region3.combinedRegion(th1);

				Region3[] tl2 = new Region3[m_capacity + 1 - l];
				Region3[] th2 = new Region3[m_capacity + 1 - l];
				...
				Region3 bbl2 = Region3.combinedRegion(tl2);
				Region3 bbh2 = Region3.combinedRegion(th2);
		...
		for (cChild = 1; cChild <= splitDistribution; cChild++)
		{
			...
			Region3 bb1 = Region3.combinedRegion(t1);

			Region3[] t2 = new Region3[m_capacity + 1 - l];
			...
			Region3 bb2 = Region3.combinedRegion(t2);
			...
			case SpatialIndex3.RtreeVariantQuadratic:
				...
				for (cChild = 0; cChild < m_capacity; cChild++)
				{
					...
					for (cIndex = cChild + 1; cIndex < m_capacity + 1; cIndex++)
					{
						...
						Region3 r = m_pMBR[cChild].combinedRegion(m_pMBR[cIndex]);
	...
	protected void condenseTree(Stack toReinsert, Stack pathBuffer)
	{
		...
		if (pathBuffer.empty())
		{
			...
			{
				Node3 n = m_pTree.readNode(m_pIdentifier[0]);
		...
		else
		{
			...
			else
			{
				...
				p.m_pMBR[child] = (Region3) m_nodeMBR.clone();
	...
	protected void load(byte[] data) throws IOException
	{
		...
		for (int cChild = 0; cChild < m_children; cChild++)
		{
			...
			Region3.combinedRegion(m_nodeMBR, m_pMBR[cChild]);
		}
	}
	...
	protected byte[] store() throws IOException
	{
		...
		int type;
		if (m_level == 0) type = SpatialIndex3.PersistentLeaf;
		else type = SpatialIndex3.PersistentIndex;
		ds.writeInt(type);
	...
	class RstarSplitEntry
	{
		...
		RstarSplitEntry(Region3 r, int id, int dimension) { m_pRegion = r; m_id = id; m_sortDim = dimension; }
	}
