...
import sil.spatialindex.Point3;
...
import sil.spatialindex.IData3;
import sil.spatialindex.IEntry3;
import sil.spatialindex.ISpatialIndex3;
import sil.spatialindex.INode3;
import sil.spatialindex.IVisitor3;
import sil.storagemanager.IStorageManager3;

public class RTree3 implements ISpatialIndex3
{
	RWLock m_rwLock;

	IStorageManager3 m_pStorageManager;
	...
	public RTree3(PropertySet3 ps, IStorageManager3 sm)
	{
		m_rwLock = new RWLock();
		m_pStorageManager = sm;
		m_rootID = IStorageManager3.NewPage;
		m_headerID = IStorageManager3.NewPage;
		m_treeVariant = SpatialIndex3.RtreeVariantRstar;
		m_fillFactor = 0.7f;
		m_indexCapacity = 100;
		m_leafCapacity = 100;
	...
	{
            System.out.println("Dimensions: "+shape.getDimension());
		if (shape.getDimension() != m_dimension) throw new IllegalArgumentException("insertData: Shape has the wrong number of dimensions."+ shape.getDimension());

		m_rwLock.write_lock();

		try
		{
			Region3 mbr = shape.getMBR();
	...
	public void containmentQuery(final IShape query, final IVisitor3 v)
	{
		...
		rangeQuery(SpatialIndex3.ContainmentQuery, query, v);
	}

	public void intersectionQuery(final IShape query, final IVisitor3 v)
	{
		...
		rangeQuery(SpatialIndex3.IntersectionQuery, query, v);
	}

	public void pointLocationQuery(final IShape query, final IVisitor3 v)
	{
		...
		Region3 r = null;
		if (query instanceof Point3)
		{
			r = new Region3((Point3) query, (Point3) query);
		}
		else if (query instanceof Region3)
		{
			r = (Region3) query;
		}
		else
		...
		rangeQuery(SpatialIndex3.IntersectionQuery, r, v);
	}

	public void nearestNeighborQuery(int k, final IShape query, final IVisitor3 v, final INearestNeighborComparator nnc)
	{
		...
		try
		{
			...
			while (queue.size() != 0)
			{
				...
				if (first.m_pEntry instanceof Node3)
				{
					n = (Node3) first.m_pEntry;
					v.visitNode((INode3) n);
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						{
							e = (IEntry3) readNode(n.m_pIdentifier[cChild]);
						}
				...
				else
				{
					...
					v.visitData((IData3) first.m_pEntry);
	...
	public void nearestNeighborQuery(int k, final IShape query, final IVisitor3 v)
	{
		if (query.getDimension() != m_dimension) throw new IllegalArgumentException("nearestNeighborQuery: Shape has the wrong number of dimensions.");
	...
	public PropertySet3 getIndexProperties()
	{
		PropertySet3 pRet = new PropertySet3();
	...
	public boolean isIndexValid()
	{
		...
		while (! st.empty())
		{
			...
			Region3 tmpRegion = (Region3) m_infiniteRegion.clone();
	...
	private void initNew(PropertySet3 ps) throws IOException
	{
		Object var;
		...
		if (var != null)
		{
			if (var instanceof Integer)
			{
				...
				if (i != SpatialIndex3.RtreeVariantLinear &&  i != SpatialIndex3.RtreeVariantQuadratic && i != SpatialIndex3.RtreeVariantRstar)
		...
		Leaf3 root = new Leaf3(this, -1);
		m_rootID = writeNode(root);

		storeHeader();
	}

	private void initOld(PropertySet3 ps) throws IOException
	{
		loadHeader();
	...
	protected void insertData_impl(byte[] pData, Region3 mbr, int id)
	{
//		assert mbr.getDimension() == m_dimension;
		...
		Node3 l = root.chooseSubtree(mbr, 0, pathBuffer);
		l.insertData(pData, mbr, id, pathBuffer, overflowTable);

		m_stats.m_data++;
	}

	protected void insertData_impl(byte[] pData, Region3 mbr, int id, int level, boolean[] overflowTable)
	{
//		assert mbr.getDimension() == m_dimension;
		...
		Node3 root = readNode(m_rootID);
		Node3 n = root.chooseSubtree(mbr, level, pathBuffer);
		n.insertData(pData, mbr, id, pathBuffer, overflowTable);
	}

	protected boolean deleteData_impl(final Region3 mbr, int id)
	{
//		assert mbr.getDimension() == m_dimension;
		...
		Node3 root = readNode(m_rootID);
		Leaf3 l = root.findLeaf(mbr, id, pathBuffer);
	...
	protected int writeNode(Node3 n) throws IllegalStateException
	{
		byte[] buffer = null;
		...
		int page;
		if (n.m_identifier < 0) page = IStorageManager3.NewPage;
	...
	protected Node3 readNode(int id)
	{
		byte[] buffer;
		...
		try
		{
			...
			if (nodeType == SpatialIndex3.PersistentIndex) n = new Index3(this, -1, 0);
			else if (nodeType == SpatialIndex3.PersistentLeaf) n = new Leaf3(this, -1);
	...
	protected void deleteNode(Node3 n)
	{
		try
		{
	...
	private void rangeQuery(int type, final IShape query, final IVisitor3 v)
	{
		m_rwLock.read_lock();
		...
		try
		{
			...
			while (! st.empty())
			{
				...
				if (n.m_level == 0)
				{
					...
					{
						boolean b;
						if (type == SpatialIndex3.ContainmentQuery) b = query.contains(n.m_pMBR[cChild]);
	...
	public String toString()
	{
		...
		if (m_treeVariant == SpatialIndex3.RtreeVariantRstar)
		{
	...
	class NNEntry
	{
		...
		NNEntry(IEntry3 e, double f) { m_pEntry = e; m_minDist = f; }
	}
	...
	{
		public double getMinimumDistance(IShape query, IEntry3 e)
		{
			IShape s = e.getShape();
	...
	class ValidateEntry
	{
		...
		ValidateEntry(Region3 r, Node3 pNode) { m_parentMBR = r; m_pNode = pNode; }
	}

	class Data implements IData3
	{
		int m_id;
		Region3 m_shape;
		byte[] m_pData;

		Data(byte[] pData, Region3 mbr, int id) { m_id = id; m_shape = mbr; m_pData = pData; }

		public int getIdentifier() { return m_id; }
		public IShape getShape() { return new Region3(m_shape); }
		public byte[] getData()
		{
