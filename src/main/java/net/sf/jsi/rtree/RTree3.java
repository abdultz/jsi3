 ...
 */
public class RTree3 implements SpatialIndex3, Serializable {
  private static final long serialVersionUID = 5946232781609920309L;
  private static final Logger log = LoggerFactory.getLogger(RTree3.class);
  private static final Logger deleteLog = LoggerFactory.getLogger(RTree3.class.getName() + "-delete");
  ...
  private TIntObjectHashMap<Node3> nodeMap = new TIntObjectHashMap<Node3>();
  ...
  //  init(Properties)
  //  add(Rectangle3, int)
  //  delete(Rectangle3, int)
  //  nearest(Point3, TIntProcedure, float)
  //  intersects(Rectangle3, TIntProcedure)
  //  contains(Rectangle3, TIntProcedure)
  //  size()
  //-------------------------------------------------------------------------
  /**
  ...
  public void init(Properties props) {
    ...
    Node3 root = new Node3(rootNodeId, 1, maxNodeEntries);
  ...
  public void add(Rectangle3 r, int id) {
    ...
    add(r.minX, r.minY, r.minZ, r.maxX, r.maxY, r.maxZ, id, 1);
   ...
   */
  private void add(float minX, float minY, float minZ, float maxX, float maxY, float maxZ, int id, int level) {
    // I1 [Find position for new record] Invoke ChooseLeaf to select a
    // leaf node L in which to place r
    Node3 n = chooseNode(minX, minY, minZ, maxX, maxY, maxZ, level);
    Node3 newLeaf = null;
    ...
    if (n.entryCount < maxNodeEntries) {
      n.addEntry(minX, minY, minZ, maxX, maxY, maxZ, id);
    } else {
      newLeaf = splitNode(n, minX, minY, minZ, maxX, maxY, maxZ, id);
    }

    // I3 [Propagate changes upwards] Invoke AdjustTree on L, also passing LL
    // if a split was performed
    Node3 newNode = adjustTree(n, newLeaf);

    // I4 [Grow tree taller] If node split propagation caused the root to
    // split, create a new root whose children are the two resulting nodes.
    if (newNode != null) {
      int oldRootNodeId = rootNodeId;
      Node3 oldRoot = getNode(oldRootNodeId);

      rootNodeId = getNextNodeId();
      treeHeight++;
      Node3 root = new Node3(rootNodeId, treeHeight, maxNodeEntries);
      root.addEntry(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ, newNode.nodeId);
      root.addEntry(oldRoot.mbrMinX, oldRoot.mbrMinY, oldRoot.mbrMinZ, oldRoot.mbrMaxX, oldRoot.mbrMaxY, oldRoot.mbrMaxZ, oldRoot.nodeId);
      nodeMap.put(rootNodeId, root);
    }
  }
  ...
  public boolean delete(Rectangle3 r, int id) {
    ...
    // all entries have been checked.
      ...
      if (!n.isLeaf()) {
        ...
        for (int i = startIndex; i < n.entryCount; i++) {
	  	    if (Rectangle3.contains(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i],
        ...
        foundIndex = n.findEntry(r.minX, r.minY, r.minZ, r.maxX, r.maxY, r.maxZ, id);
      }
    ...
    if (size == 0) {
      ...
      root.mbrMinZ = Float.MAX_VALUE;
      root.mbrMaxX = -Float.MAX_VALUE;
      root.mbrMaxY = -Float.MAX_VALUE;
      root.mbrMaxZ = -Float.MAX_VALUE;
    }
  ...
  public void nearest(Point3 p, TIntProcedure v, float furthestDistance) {
    Node3 rootNode = getNode(rootNodeId);
  ...
  private void createNearestNDistanceQueue(Point3 p, int count, PriorityQueue distanceQueue, float furthestDistance) {
    ...
    while (parents.size() > 0) {
      ...
      if (!n.isLeaf()) {
        ...
        boolean near = false;
        for (int i = startIndex; i < n.entryCount; i++) {
          if (Rectangle3.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i],
                                 n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i],
                                 p.x, p.y, p.z) <= furthestDistanceSq) {
            parents.push(n.ids[i]);
            parentsEntry.pop();
            parentsEntry.push(i); // this becomes the start index when the child has been searched
            parentsEntry.push(-1);
        ...
        for (int i = 0; i < n.entryCount; i++) {
          float entryDistanceSq = Rectangle3.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i],
                                                   n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i],
                                                   p.x, p.y, p.z);
          int entryId = n.ids[i];
   ...
   */
  public void nearestNUnsorted(Point3 p, TIntProcedure v, int count, float furthestDistance) {
   ...
   */
  public void nearestN(Point3 p, TIntProcedure v, int count, float furthestDistance) {
    PriorityQueue distanceQueue = new PriorityQueue(PriorityQueue.SORT_ORDER_DESCENDING);
    createNearestNDistanceQueue(p, count, distanceQueue, furthestDistance);
   ...
   */
  public void intersects(Rectangle3 r, TIntProcedure v) {
    Node3 rootNode = getNode(rootNodeId);
   ...
   */
  public void contains(Rectangle3 r, TIntProcedure v) {
    ...
    while (parents.size() > 0) {
      ...
      if (!n.isLeaf()) {
        ...
        for (int i = startIndex; i < n.entryCount; i++) {
          if (Rectangle3.intersects(r.minX, r.minY, r.minZ, r.maxX, r.maxY, r.maxZ,
                                   n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i])) {
            parents.push(n.ids[i]);
            parentsEntry.pop();
            parentsEntry.push(i); // this becomes the start index when the child has been searched
            parentsEntry.push(-1);
        ...
        for (int i = 0; i < n.entryCount; i++) {
          if (Rectangle3.contains(r.minX, r.minY, r.minZ, r.maxX, r.maxY, r.maxZ,
                                 n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i])) {
   ...
   */
  private Node3 splitNode(Node3 n, float newRectMinX, float newRectMinY, float newRectMinZ, float newRectMaxX, float newRectMaxY, float newRectMaxZ, int newId) {
    ...
    if (log.isDebugEnabled()) {
      ...
      float unionMinZ = Math.min(n.mbrMinZ, newRectMinZ);
      float unionMaxX = Math.max(n.mbrMaxX, newRectMaxX);
      float unionMaxY = Math.max(n.mbrMaxY, newRectMaxY);
      float unionMaxZ = Math.max(n.mbrMaxZ, newRectMaxZ);

      initialArea = (unionMaxX - unionMinX) * (unionMaxY - unionMinY) * (unionMaxZ - unionMinZ);
    }

    System.arraycopy(initialEntryStatus, 0, entryStatus, 0, maxNodeEntries);

    Node3 newNode = null;
    newNode = new Node3(getNextNodeId(), n.level, maxNodeEntries);
    nodeMap.put(newNode.nodeId, newNode);

    pickSeeds(n, newRectMinX, newRectMinY, newRectMinZ, newRectMaxX, newRectMaxY, newRectMaxZ, newId, newNode); // this also sets the entryCount to 1
    ...
    while (n.entryCount + newNode.entryCount < maxNodeEntries + 1) {
      if (maxNodeEntries + 1 - newNode.entryCount == minNodeEntries) {
        ...
        for (int i = 0; i < maxNodeEntries; i++) {
          if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
            ...
            if (n.entriesMinZ[i] < n.mbrMinZ) n.mbrMinZ = n.entriesMinZ[i];
            if (n.entriesMaxX[i] > n.mbrMaxX) n.mbrMaxX = n.entriesMaxX[i];
            if (n.entriesMaxY[i] > n.mbrMaxY) n.mbrMaxY = n.entriesMaxY[i];
            if (n.entriesMaxZ[i] > n.mbrMaxZ) n.mbrMaxZ = n.entriesMaxZ[i];
      ...
      if (maxNodeEntries + 1 - n.entryCount == minNodeEntries) {
        ...
        for (int i = 0; i < maxNodeEntries; i++) {
          if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
            entryStatus[i] = ENTRY_STATUS_ASSIGNED;
            newNode.addEntry(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i], n.ids[i]);
    ...
    if (INTERNAL_CONSISTENCY_CHECKING) {
      Rectangle3 nMBR = new Rectangle3(n.mbrMinX, n.mbrMinY,  n.mbrMinZ, n.mbrMaxX, n.mbrMaxY, n.mbrMaxZ);
      if (!nMBR.equals(calculateMBR(n))) {
        log.error("Error: splitNode old node MBR wrong");
      }
      Rectangle3 newNodeMBR = new Rectangle3(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ);
    ...
    // debug code
    if (log.isDebugEnabled()) {
      float newArea = Rectangle3.area(n.mbrMinX, n.mbrMinY, n.mbrMinZ, n.mbrMaxX, n.mbrMaxY, n.mbrMaxZ) +
                      Rectangle3.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ);
   ...
   */
  private void pickSeeds(Node3 n, float newRectMinX, float newRectMinY,  float newRectMinZ, float newRectMaxX, float newRectMaxY, float newRectMaxZ, int newId, Node3 newNode) {
    ...
    tempHighestLow = newRectMinZ;
    tempHighestLowIndex = -1; // -1 indicates the new rectangle is the seed

    tempLowestHigh = newRectMaxZ;
    tempLowestHighIndex = -1; // -1 indicates the new rectangle is the seed

    for (int i = 0; i < n.entryCount; i++) {
      float tempLow = n.entriesMinZ[i];
      if (tempLow >= tempHighestLow) {
         tempHighestLow = tempLow;
         tempHighestLowIndex = i;
      } else {  // ensure that the same index cannot be both lowestHigh and highestLow
        float tempHigh = n.entriesMaxZ[i];
        if (tempHigh <= tempLowestHigh) {
          tempLowestHigh = tempHigh;
          tempLowestHighIndex = i;
        }
      ...
      // PS2 [Adjust for shape of the rectangle cluster] Normalize the separations
      // by dividing by the depths of the entire set along the corresponding
      // dimension
      float normalizedSeparation = mbrLenZ == 0 ? 1 : (tempHighestLow - tempLowestHigh) / mbrLenZ;
      if (normalizedSeparation > 1 || normalizedSeparation < -1) {
        log.error("Invalid normalized separation Z");
      }

      if (log.isDebugEnabled()) {
        log.debug("Entry " + i + ", dimension Z: HighestLow = " + tempHighestLow +
                  " (index " + tempHighestLowIndex + ")" + ", LowestHigh = " +
                  tempLowestHigh + " (index " + tempLowestHighIndex + ", NormalizedSeparation = " + normalizedSeparation);
      }
...
//check this
    // At this point it is possible that the new rectangle3 is both highestLow and lowestHigh.
...
//-check this
    ...
    if (highestLowIndex == -1) {
      newNode.addEntry(newRectMinX, newRectMinY, newRectMinZ, newRectMaxX, newRectMaxY, newRectMaxZ, newId);
    } else {
      newNode.addEntry(n.entriesMinX[highestLowIndex], n.entriesMinY[highestLowIndex], n.entriesMinZ[highestLowIndex],
                       n.entriesMaxX[highestLowIndex], n.entriesMaxY[highestLowIndex], n.entriesMaxZ[highestLowIndex],
      ...
      n.entriesMinZ[highestLowIndex] = newRectMinZ;
      n.entriesMaxX[highestLowIndex] = newRectMaxX;
      n.entriesMaxY[highestLowIndex] = newRectMaxY;
      n.entriesMaxZ[highestLowIndex] = newRectMaxZ;
    ...
    n.mbrMinZ = n.entriesMinZ[lowestHighIndex];
    n.mbrMaxX = n.entriesMaxX[lowestHighIndex];
    n.mbrMaxY = n.entriesMaxY[lowestHighIndex];
    n.mbrMaxZ = n.entriesMaxZ[lowestHighIndex];
  }
   ...
   */
  private int pickNext(Node3 n, Node3 newNode) {
    ...
    for (int i = 0; i < maxNodeEntries; i++) {
      if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
        ...
        float nIncrease = Rectangle3.enlargement(n.mbrMinX, n.mbrMinY, n.mbrMinZ, n.mbrMaxX, n.mbrMaxY, n.mbrMaxZ,
                                                n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i]);
        float newNodeIncrease = Rectangle3.enlargement(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ,
                                                      n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i]);
        ...
        if (difference > maxDifference) {
          ...
          if (nIncrease < newNodeIncrease) {
            ...
            nextGroup = 1;
          } else if (Rectangle3.area(n.mbrMinX, n.mbrMinY, n.mbrMinZ, n.mbrMaxX, n.mbrMaxY, n.mbrMaxZ) < Rectangle3.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ)) {
            nextGroup = 0;
          } else if (Rectangle3.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMinZ, newNode.mbrMaxX, newNode.mbrMaxY, newNode.mbrMaxZ) < Rectangle3.area(n.mbrMinX, n.mbrMinY, n.mbrMinZ, n.mbrMaxX, n.mbrMaxY, n.mbrMaxZ)) {
            nextGroup = 1;
    ...
    if (nextGroup == 0) {
      ...
      if (n.entriesMinZ[next] < n.mbrMinZ) n.mbrMinZ = n.entriesMinZ[next];
      if (n.entriesMaxX[next] > n.mbrMaxX) n.mbrMaxX = n.entriesMaxX[next];
      if (n.entriesMaxY[next] > n.mbrMaxY) n.mbrMaxY = n.entriesMaxY[next];
      if (n.entriesMaxZ[next] > n.mbrMaxZ) n.mbrMaxZ = n.entriesMaxZ[next];
      n.entryCount++;
    } else {
      // move to new node.
      newNode.addEntry(n.entriesMinX[next], n.entriesMinY[next], n.entriesMinZ[next], n.entriesMaxX[next], n.entriesMaxY[next], n.entriesMaxZ[next], n.ids[next]);
      n.ids[next] = -1;
    }
   ...
   */
  private float nearest(Point3 p, Node3 n, float furthestDistanceSq, TIntArrayList nearestIds) {
    for (int i = 0; i < n.entryCount; i++) {
      float tempDistanceSq = Rectangle3.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i], p.x, p.y, p.z);
   ...
   */
  private boolean intersects(Rectangle3 r, TIntProcedure v, Node3 n) {
    for (int i = 0; i < n.entryCount; i++) {
      if (Rectangle3.intersects(r.minX, r.minY, r.minZ, r.maxX, r.maxY, r.maxZ, n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i])) {
        if (n.isLeaf()) {
          if (!v.execute(n.ids[i])) {
            return false;
          }
        } else {
          Node3 childNode = getNode(n.ids[i]);
   ...
   */
  private void condenseTree(Node3 l) {
    ...
    while (n.level != treeHeight) {
      ...
      if (n.entryCount < minNodeEntries) {
        ...
        if (n.mbrMinX != parent.entriesMinX[parentEntry] ||
          ...
          parent.recalculateMBRIfInfluencedBy(deletedMinX, deletedMinY, deletedMinZ, deletedMaxX, deletedMaxY, deletedMaxZ);
        }
      }
    ...
    while (eliminatedNodeIds.size() > 0) {
      Node3 e = getNode(eliminatedNodeIds.pop());
      for (int j = 0; j < e.entryCount; j++) {
        add(e.entriesMinX[j], e.entriesMinY[j], e.entriesMinZ[j], e.entriesMaxX[j], e.entriesMaxY[j], e.entriesMaxZ[j], e.ids[j], e.level);
        e.ids[j] = -1;
      }
      e.entryCount = 0;
   ...
   */
  private Node3 chooseNode(float minX, float minY, float minZ, float maxX, float maxY, float maxZ,int level) {
    // CL1 [Initialize] Set N to be the root node
    Node3 n = getNode(rootNodeId);
    parents.clear();
    parentsEntry.clear();
    ...
    while (true) {
      ...
      float leastEnlargement = Rectangle3.enlargement(n.entriesMinX[0], n.entriesMinY[0], n.entriesMinZ[0], n.entriesMaxX[0], n.entriesMaxY[0], n.entriesMaxZ[0],
                                                     minX, minY, minZ, maxX, maxY, maxZ);
      int index = 0; // index of rectangle in subtree
      for (int i = 1; i < n.entryCount; i++) {
        float tempMinX = n.entriesMinX[i];
        float tempMinY = n.entriesMinY[i];
        float tempMinZ = n.entriesMinZ[i];
        float tempMaxX = n.entriesMaxX[i];
        float tempMaxY = n.entriesMaxY[i];
        float tempMaxZ = n.entriesMaxZ[i];
        float tempEnlargement = Rectangle3.enlargement(tempMinX, tempMinY, tempMinZ, tempMaxX, tempMaxY, tempMaxZ,
                                                      minX, minY, minZ, maxX, maxY, maxZ);
        if ((tempEnlargement < leastEnlargement) ||
            ((tempEnlargement == leastEnlargement) &&
             (Rectangle3.area(tempMinX, tempMinY, tempMinZ, tempMaxX, tempMaxY, tempMaxZ) <
              Rectangle3.area(n.entriesMinX[index], n.entriesMinY[index], n.entriesMinZ[index], n.entriesMaxX[index], n.entriesMaxY[index], n.entriesMaxZ[index])))) {
          index = i;
   ...
   */
  private Node3 adjustTree(Node3 n, Node3 nn) {
    ...
    while (n.level != treeHeight) {
      ...
      Node3 parent = getNode(parents.pop());
      ...
      if (parent.entriesMinX[entry] != n.mbrMinX ||
          ...
          parent.entriesMinZ[entry] != n.mbrMinZ ||
          parent.entriesMaxX[entry] != n.mbrMaxX ||
          parent.entriesMaxY[entry] != n.mbrMaxY ||
          parent.entriesMaxZ[entry] != n.mbrMaxZ) {
      ...
      if (nn != null) {
        if (parent.entryCount < maxNodeEntries) {
          parent.addEntry(nn.mbrMinX, nn.mbrMinY, nn.mbrMinZ, nn.mbrMaxX, nn.mbrMaxY, nn.mbrMaxZ, nn.nodeId);
        } else {
          newNode = splitNode(parent, nn.mbrMinX, nn.mbrMinY, nn.mbrMinZ, nn.mbrMaxX, nn.mbrMaxY, nn.mbrMaxZ, nn.nodeId);
        }
      }
  ...
  private boolean checkConsistency(int nodeId, int expectedLevel, Rectangle3 expectedMBR) {
    // go through the tree, and check that the internal data structures of
    // the tree are not corrupted.
    Node3 n = getNode(nodeId);
    ...
    Rectangle3 calculatedMBR = calculateMBR(n);
    Rectangle3 actualMBR = new Rectangle3();
    actualMBR.minX = n.mbrMinX;
    actualMBR.minY = n.mbrMinY;
    actualMBR.minZ = n.mbrMinZ;
    actualMBR.maxX = n.mbrMaxX;
    actualMBR.maxY = n.mbrMaxY;
    actualMBR.maxZ = n.mbrMaxZ;
    if (!actualMBR.equals(calculatedMBR)) {
      ...
      if (actualMBR.minZ != n.mbrMinZ) log.error("  actualMinZ=" + actualMBR.minZ + ", calc=" + calculatedMBR.minZ);
      ...
      if (actualMBR.maxZ != n.mbrMaxZ) log.error("  actualMaxZ=" + actualMBR.maxZ + ", calc=" + calculatedMBR.maxZ);
      return false;
    }
    ...
    for (int i = 0; i < n.entryCount; i++) {
      ...
      if (n.level > 1) { // if not a leaf
        if (!checkConsistency(n.ids[i], n.level - 1, new Rectangle3(n.entriesMinX[i], n.entriesMinY[i], n.entriesMinZ[i], n.entriesMaxX[i], n.entriesMaxY[i], n.entriesMaxZ[i]))) {
          return false;
        }
      }
    }
